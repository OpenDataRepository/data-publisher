...
//        fwrite(STDERR, 'Search Key: '.$search_key."\n");
        $grandparent_datarecord_list = $search_api_service->performSearch(
            ...
            $search_key,
            [],      // search testing is with either zero permissions, or super-admin permissions
            false,        // only want grandparent datarecord ids here
            [],      // testing doesn't need a specific set of sort datafields...
            [],      // ...or a specific sort order
...
//        fwrite(STDERR, 'Search Key: '.$search_key."\n");
        $complete_datarecord_list = $search_api_service->performSearch(
            ...
            [],      // the complete datarecord list can't be sorted
            [],
            $search_as_super_admin
        );
...
//        fwrite(STDERR, 'Search Key: '.$search_key."\n");
    ...
    public function testFilter($search_params, $expected_search_params, $search_as_super_admin)
    {
        ...
        $filtered_search_key = $search_api_service->filterSearchKeyForUser($datatype_id, $search_key, [], $search_as_super_admin);
    ...
    public function provideSearchParams()
    {
        return [
            ...
            'RRUFF Reference: article title containing "structure"' => [
                ...
                [1,4,6,7,10,12,23,25,35,46,65,69,72,82,85,87,89],
                true
            ],
            ...
            'RRUFF Reference: article title containing "abelsonite" OR "structure" (variant 1)' => [
                ...
                [1,4,6,7,10,12,23,25,35,46,63,65,69,72,82,83,85,87,89],
                true
            ],
            ...
            'IMA List: records without dates' => [
                ...
                [93,94,96,97,322],    // NOTE: 93 was set to have a date, then cleared...the database stores the "empty" value as 9999-12-31
                true
            ],
            ...
            'IMA List: records with date on or after 1805-06-30' => [
                ...
                [91,92,95],    // 92 is included because 'starting from 1805-06-30' includes the date
                true
            ],
            ...
            'IMA List: records with date between 1805-06-30 and 2017-10-01' => [
                ...
                [91,92,95],    // 95 is included because users expect to include the later date when both exist
                true
            ],
            ...
            'RRUFF Sample: samples where Raman Spectra::wavelength contains "780"' => [
                ...
                [102,118,107,125,109,126,131,124,123,103,120,119,100,105],
                true
            ],
            ...
            'RRUFF Sample: sample_descriptions containing "z OR \"", without non-public records' => [
                ...
                [102,115,127,132,134,110,99,135,104],
                false
            ],
            ...
            'IMA List: mineral_id contains "7 abc"' => [
                ...
                [91,94,96],    // the "7" token should end up matching mineral_ids 777, 788, and 790
                true
            ],
            'IMA List: mineral_id contains "7 abc", without non-public records' => [
                ...
                [94,96],    // the mineral_id 777 is non-public, so it won't be in a search done without permissions
                false
            ],
            ...
            'RRUFF Reference: article_title containing "<i>"' => [
                ...
                [56],   // this test is the entire point of this block...need to confirm that the parser can guess it's seeing HTML tags
                true
            ],
            ...
            'RRUFF Reference: article_title containing "<i >g"' => [
                ...
                [8,11],    // this should only get references with an article title that start with an 'h' or 'H'...but not trigger the title containing '<i>'
                true
            ],
...
//            ],
            ...
            'XYZData test, advanced: search with both x/y that should return nothing' => [
                ...
                [],    // the y value is between 30 and 35 for the previous matches, so nothing should match here
                true
            ],
            ...
            'XYZData test, advanced: multirange x/y with alternate y' => [
                ...
                [325,327,328],  // 328 matches now because its xvalues between 1.4 and 1.5 had yvalues between 4.26 and 5.47
                true
            ],
...
//            ],
            ...
            'XYZData test, simple: multirange x/y with alternate y (1)' => [
                ...
                [325,327,328],  // this should get converted into "(>=1.4 <=1.5,>=7 OR <=5,)|(>=2.4 <=2.5,>=7 OR <=5,)"...328 matches now because its xvalues between 1.4 and 1.5 had yvalues between 4.26 and 5.47
                true
            ],
            'XYZData test, simple: multirange x/y with alternate y (2)' => [
                ...
                [325,327,328],  // like above, but the alternate OR syntax needs conversion so <=5 doesn't become a z value
                true
            ],
    ...
    public function provideSearchParamsCompleteDatarecordList()
    {
        return [
            ...
            'RRUFF Sample: default search for complete datarecord list' => [
                ...
                array_merge( range(1, 295), [322,323] ),    // all RRUFF Reference records, plus the 8 IMA List records, plus the 43 RRUFF Sample records, plus the 156 Raman Spectra records
                true
            ],
    ...
    public function provideInverseSearchParams()
    {
        ...
        return [
            ...
            'RRUFF Reference: inverse search, references with the rruff_id "R050111"' => [
                ...
                [5,13,22,26,34,41,80],    // should be no difference from previous...this sample doesn't link to a reference
                true
            ],
            ...
            'IMA List: inverse search, mineral with a mineral_name "Ab"' => [
                ...
                [91,95],  // should still match "Abelsonite" and "Abellaite"
                true
            ],
            'IMA List: inverse search, minerals with reference author "downs"' => [
                ...
                [91,94,97],  // Abelsonite, Aegirine, and Anorthite
                true
            ],
            'IMA List: inverse search, minerals with a wavelength "514"' => [
                ...
                [92,93,94,97],  // Bournonite, Amesite, Aegirine, and Anorthite all have 514 spectra
                true
            ],
    ...
    public function provideIgnoreDescendantsSearchParams()
    {
        ...
        return [
            ...
            'RRUFF Sample: search for article title not blank, but ignore IMA Mineral link' => [
                ...
                [107, 126],  // These are the only two records from RRUFF Sample that link directly to RRUFF Reference
                true
            ],
            ...
            'RRUFF Sample: search for article title containing "Abelsonite", but ignore the direct link' => [
                ...
                [98, /*126*/],  // 98 has the reference linked via IMA, while 126 has the reference directly linked
                true
            ],
            ...
            'RRUFF Sample: search for mineral name containing "b", but ignore IMA Mineral Link' => [
                ...
                [],  // Will be no results due to ignoring IMA Mineral
                true
            ],
        ];
    }
    ...
    public function provideORSearchParams()
    {
        ...
        return [
            ...
            'RRUFF Reference: search for pyroxene' => [
                ...
                [3,10,31,32,36,38,43,65,68,70,72,73],
                true
            ],
    ...
    public function provideSortSearchParams()
    {
        ...
        return [
            ...
            'IMA List: sort asc by mineral name' => [
                [
                    'dt_id' => 2,
                    'sort_by' => ["sort_df_id" => "17", "sort_dir" => "asc"],
                ],
                [95,91,96,94,93,97,92,322],    // order matters now
                true
            ],
            'IMA List: sort desc by mineral name' => [
                [
                    'dt_id' => 2,
                    'sort_by' => ["sort_df_id" => "17", "sort_dir" => "desc"],
                ],
                [322,92,97,93,94,96,91,95],
                true
            ],
            'IMA List: sort asc by mineral id' => [
                [
                    'dt_id' => 2,
                    'sort_by' => ["sort_df_id" => "16", "sort_dir" => "asc"],
                ],
                [91,96,94,93,97,92,95,322],
                true
            ],
            ...
            'IMA List: sort asc by country, then asc by mineral name' => [
                [
                    'dt_id' => 2,
                    'sort_by' => [
                        ["sort_df_id" => "24", "sort_dir" => "asc"],
                        ["sort_df_id" => "17", "sort_dir" => "asc"]
                    ],
                ],
                [322,97,94,95,96,92,91,93],
                true
            ],
            'IMA List: sort asc by country, then desc by mineral name' => [
                [
                    ...
                    'sort_by' => [
                        ["sort_df_id" => "24", "sort_dir" => "asc"],
                        ["sort_df_id" => "17", "sort_dir" => "desc"]
                    ],
                ],
                [322,97,94,95,96,92,93,91],
                true
            ],
        ];
    }
...
//                    '64' => '!""',    // ...this field should not
            ...
            'IMA List: sort by date_first_published, with permissions' => [
                [
                    'dt_id' => 2,
                    'sort_by' => ["sort_df_id" => "64", "sort_dir" => "asc"],    // NOTE: ODR will read this format for 'sort_by'...
                ],
                [
                    'dt_id' => 2,
                    'sort_by' => [  // ...but it will turn 'sort_by' into this format because of multi-datafield searching
            ...
            'IMA List: sort by date_first_published, without permissions' => [
                [
                    'dt_id' => 2,
                    'sort_by' => ["sort_df_id" => "64", "sort_dir" => "asc"],    // date_first_published is not public...
                ],
                [
            ...
            'IMA List: sort by mineral_name and date_first_published, without permissions' => [
                [
                    ...
                    'sort_by' => [
                        ["sort_df_id" => "64", "sort_dir" => "asc"],
                        ["sort_df_id" => "17", "sort_dir" => "asc"],    // need to ensure this works when only one sort criteria is getting filtered
                    ],
                ],
                [
                    'dt_id' => 2,
