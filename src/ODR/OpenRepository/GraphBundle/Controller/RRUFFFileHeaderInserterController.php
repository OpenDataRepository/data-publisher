 ...
 * RRUFF FileHeaderInserter Controller
...
class RRUFFFileHeaderInserterController extends ODRCustomController
{
    ...
    public function rebuildallAction($dr_id, Request $request)
    {
        ...
        try {
            ...
            //  relevant plugins
            $relevant_plugins = array('odr_plugins.rruff.file_header_inserter' => 1);
            $conflicting_plugins = array('odr_plugins.base.file_header_inserter' => 1);
                ...
                if ( isset($df['renderPluginInstances']) ) {
                    foreach ($df['renderPluginInstances'] as $rpi_id => $rpi) {
                        ...
                        if ( $rpi['renderPlugin']['pluginClassName'] === 'odr_plugins.base.file_header_inserter' ) {
                            // Datafield is also using the FileHeaderInserter plugin...
                            $datafield_name = $df['dataFieldMeta']['fieldName'];
                            throw new ODRException('The datafield "'.$datafield_name.'" should not have both the Base FileHeaderInserter and the RRUFF FileHeaderInserter plugin active simultaneously');
                        }
                    }
