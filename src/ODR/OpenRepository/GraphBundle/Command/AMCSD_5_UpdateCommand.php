...
class AMCSD_5_UpdateCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        ...
        $this
            ->setName('odr_amcsd_update:5_update')
            ->setDescription('TODO');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_amcsd_update:5_update] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
            ...
            try {
                ...
                $output->writeln('AMCSD_5_UpdateCommand (user '.$user_id.') ('.$redis_prefix.'): '.$current_time->format('Y-m-d H:i:s').' (UTC-5)');
                $logger->info('AMCSD_5_UpdateCommand.php (user '.$user_id.') ('.$redis_prefix.'): updating AMCSD...');

                /** @var AMCSDUpdateService $amcsd_update_service */
                $amcsd_update_service = $container->get('odr.amcsd_update_service');
                $ret = $amcsd_update_service->amcsdupdateAction($user_id, $output);
                ...
                if ( $e->getMessage() == 'retry' ) {
                    ...
                    $logger->error('AMCSD_5_UpdateCommand.php: '.$e->getMessage());
