...
class RRUFFSamplePlugin implements DatatypePluginInterface
{
     ...
     */
    public function __construct(private readonly EntityManager $em, private readonly EntityCreationService $entity_create_service, private readonly LockService $lock_service, private readonly EventDispatcherInterface $event_dispatcher, private readonly CsrfTokenManager $token_manager, private readonly \Twig\Environment $templating, private readonly LoggerInterface $logger)
    {
    }
    ...
    public function onDatarecordCreate(DatarecordCreatedEvent $event)
    {
        ...
        $query = $this->em->createQuery(
           'SELECT rpf.fieldName, df
            ...
            [
                ...
                'field_names' => ['Sample ID', 'RRUFF ID'],
            ]
        );
        ...
        foreach ($fields as $rpf_name => $df) {
            ...
            $this->logger->debug('Setting df '.$df->getId().' "Sample ID" of new dr '.$datarecord->getId().' to "'.$new_value.'"...', [self::class, 'onDatarecordCreate()']);
        }
