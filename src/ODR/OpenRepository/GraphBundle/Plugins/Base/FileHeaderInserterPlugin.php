 ...
 *
 * If you're paying attention, then you should realize that getting these various pieces of data only
 * really works when you have a database with a known/pre-defined structure...and unfortunately,
 * *THE PRIMARY DESIGN PRINCIPLE* of ODR is that users need to be able to change the structure of
 * their database, and it needs to keep working even if they change stuff on a whim.
 *
 *
 ...
 * fields are usually not in the same database, so the plugin has to go out of its way to locate fields
 * that the user is allowed to use.  Unlike the File Renamer Plugin, these fields can't be restricted
 * so this plugin has at most one value per field...so it has to also potentially choose between
 * multiple values.
 *
 ...
 * it's effectively impossible for changes made to these other fields can't automatically update the
 * relevant uploaded files...though making that happen even in a specialist system would be difficult.
 *
 * Additionally, automatically checking whether the header is out of sync is incredibly expensive,
 * as it requires the file to be decrypted and then read...and ODR is already slow enough.  As such,
 * there's no way to notify the user when a header would need updating.  The next easiest method
 * involves hijacking the Edit and MassEdit controllers so the user at least has the option of manually
 * triggering a rewrite...and it still requires additional events and controllers to work correctly.
 *
...
class FileHeaderInserterPlugin implements DatafieldPluginInterface, PluginSettingsDialogOverrideInterface, PostMassEditEventInterface
{
...
//                );
     ...
     * );
     *
     * The '<'prefix' entry is a string of datatype ids which indicate the "ultimate ancestor" of
     * a record that is is having one of its uploaded files modified.  It also assists with actually
     * finding the correct values to use in the file header later on...it will always have at least
     * one datatype id in it.
     *
     * The rest of the options are more self-explanatory...the 'comment prefix' value is what's
     * prepended to each line of the 'header' value to indicate they're not "data" lines...the
     * 'placeholder' value indicates where the values should be inserted, and the 'fields' array
     * is a shortcut to the datafield ids listed in the header.
     *
     ...
     *
     * The 'allowed_extenions' is an array of strings for which files the plugin is allowed to run
     * on.  Due to inserting lines of text into files, the plugin will wreck files which aren't
     * plain-text...such as PDF or JPEG files...but it also doesn't prevent the user from doing so
        ...
        if ( !empty($df['renderPluginInstances']) ) {
            ...
            foreach ($df['renderPluginInstances'] as $rpi_id => $rpi) {
                if ( $rpi['renderPlugin']['pluginClassName'] === 'odr_plugins.base.file_header_inserter' ) {
                    ...
                    //  no "\r" characters in there...tends to be after the plugin is saved...
     ...
     * that can have at most one record.  While I would greatly prefer this to work identically to
     * the FileRenamer Plugin, the requirements of RRUFF make that impossible.
     *
...
//                            }
     ...
     * contain all the possible values the header could use once it's loaded from the cache entries.
     *
    ...
    public function getFileHeader($original_drf, $config_info)
    {
        ...
        // This plugin has no way to ensure the datafields entered by the user are valid, so there's
        //  nothing stopping them from entering ids of datafields which...
...
//                case 'Tags':
    ...
    /**
     * Attempts to insert/verity a header for a file before it actually gets encrypted by the other
     * parts of ODR.
     *
    ...
    public function onPostMassEdit(PostMassEditEvent $event)
    {
        ...
        try {
            ...
            if ( $is_event_relevant ) {
                ...
                $this->logger->debug('Received request to update headers for files in datafield '.$datafield->getId().' datarecord '.$datarecord->getId(), array(self::class, 'onPostMassEdit()', 'drf '.$drf->getId()));
            ...
            // Would prefer if these happened regardless of success/failure...
            if ( $is_event_relevant )
                $this->logger->debug('finished header insertion attempt for the '.$typeclass.'s in datafield '.$datafield->getId().' datarecord '.$datarecord->getId(), array(self::class, 'onPostMassEdit()', 'drf '.$drf->getId()));
