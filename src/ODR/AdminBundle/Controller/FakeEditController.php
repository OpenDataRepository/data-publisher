...
class FakeEditController extends ODRCustomController
{
     ...
     * created through {@link EditController::adddatarecordAction()}, which leads to an increasing
     * number of entirely blank records in databases...
     *
     ...
     * datarecord with those values, after verifying that they're all valid.
     *
     * @param Request $request
     *
     * @return Response
...
//print_r($post);  exit();
                ...
                foreach ($datafields as $id => $val) {
                    if ( !is_array($val) ) {
                        ...
                        foreach ($val as $ro_id => $ro_val)
                            $datafields[$id][$ro_id] = stripslashes((string) $ro_val);
                    }
                }
            }
            ...
            if ( isset($post['inline_link']) )
                $inline_link = true;
            $linked_ancestor_datarecord_id = null;
            if ( isset($post['linked_ancestor_datarecord_id']) )
                $linked_ancestor_datarecord_id = $post['linked_ancestor_datarecord_id'];
            ...
            $linked_ancestor_datarecord = null;
            $linked_ancestor_datatype = null;
            ...
            if ( !is_null($linked_ancestor_datarecord_id) ) {
                ...
                /** @var DataType|null $linked_ancestor_datatype */
                $linked_ancestor_datatype = $linked_ancestor_datarecord->getDataType();
            }
            ...
            if ( !is_null($linked_ancestor_datarecord) && !$permissions_service->canEditDatarecord($user, $linked_ancestor_datarecord) )
                throw new ODRForbiddenException();
            // --------------------

            // If the new record is supposed to become a linked descendant afterwards...
            if ( !is_null($linked_ancestor_datarecord) ) {
                // ...ensure the provided linked ancestor record can have linked descendants of this
                //  datatype
                $datatree_array = $datatree_info_service->getDatatreeArray();

                if ( isset($datatree_array['linked_from'][$grandparent_datatype_id])
                    && in_array($linked_ancestor_datatype->getId(), $datatree_array['linked_from'][$grandparent_datatype_id])
                ) {
                    // This is a legal link, so do nothing...this function won't actually create
                    //  said link due to the complexities involved, though
                }
                else {
                    throw new ODRBadRequestException('The Datatype '.$linked_ancestor_datatype->getId().' cannot link to the new record');
                }
            }

            // Need to verify that the datafields and tokens make sense
            $datatype_array = $database_info_service->getDatatypeArray($grandparent_datatype_id, false);    // don't need links
            $found_datafields = [];
            ...
            foreach ($datatype_array[$datatype->getId()]['dataFields'] as $df_id => $df) {
                ...
                if ( $df['dataFieldMeta']['prevent_user_edits'] === true ) {
                    ...
                    if ( isset($special_tokens[$df_id]) ) {
                        ...
                        if ( !$this->isCsrfTokenValid($token_id, $special_tokens[$df_id]) )
                            throw new ODRBadRequestException('Invalid special CSRF Token');
                ...
                if ( isset($datafields[$df_id]) ) {
                    ...
                    // ...and that it's valid
                    if ( !$this->isCsrfTokenValid($token_id, $csrf_tokens[$df_id]) )
...
//                    throw $e;
            ...
            foreach ($datafields as $df_id => $value) {
                ...
                if ( $typeclass === 'Radio' ) {
                    ...
                    // Every tag listed in this array needs to be selected
                    $selections = [];
                    foreach ($value as $tag_id => $num)
                        $selections[$tag_id] = 1;

                    // Ensure a drf entry exists...
                    $drf = $entity_create_service->createDatarecordField($user, $new_datarecord, $df);
                    // ...and select the requested tags
        ...
        foreach ($dt_array as $dt_id => $dt) {
            ...
            foreach ($dt['renderPluginInstances'] as $rpi_id => $rpi) {
                foreach ($rpi['renderPluginMap'] as $rpf_name => $rpm_df) {
                    $rpm_df_id = $rpm_df['id'];
                    if ( isset($rpm_df['properties']['autogenerate_values']) )
...
//                    $typename = $df['dataFieldMeta']['fieldType']['typeName'];
                    ...
                    // ...and that it's valid
                    if ( !$this->isCsrfTokenValid($token_id, $csrf_tokens[$df_id]) )
                        throw new ODRBadRequestException('Invalid reload CSRF Token');
