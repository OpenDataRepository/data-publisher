...
class APIController extends ODRCustomController
{
    ...
    public function jsonLDAction($dataset_uuid, $version, Request $request)
    {
        try {
            ...
            $output_dataset["creator"] = [];
            $output_dataset["creator"]["@list"] = [];
    ...
    public function getDatatypeListAction($version, $type, Request $request)
    {
        try {
            ...
            $final_datatype_data = ['databases' => array_values($final_datatype_data)];
    ...
    public function getRecentlyModifiedFilesListAction($version, $datatype_uuid, $limit, $offset, $recent, Request $request)
    {
        try {
            ...
            */
            ...
            $params = ['datarecord_ids' => $descendants];
            // return new JsonResponse(['descendants' => count($descendants)]);
    ...
    public function getDatarecordListAction($version, $datatype_uuid, $limit, $offset, $recent, Request $request)
    {
        try {
            ...
            // The list needs to be wrapped in another array...
            $final_datarecord_list = ['records' => $final_datarecord_list];
...
//                throw new ODRForbiddenException();
            ...
            $datatype_list = ['databases' => array_slice($datatype_list, $offset, $limit)];
    ...
    public function createrecordAction($version, $dataset_uuid, Request $request)
    {
        ...
        try {
            ...
            if ($user->hasRole('ROLE_SUPER_ADMIN') && $user_email !== null) {
                ...
                /** @var ODRUser $user */
                $user = $user_manager->findUserBy(['email' => $user_email]);
                if (is_null($user))
...
//                    throw $e;
            ...
            $url = $this->generateUrl('odr_api_get_dataset_record', [
                'version' => $version,
            ...
            $logger->info('Field Section');
                    ...
                    if (isset( $field['files']) && is_array($field['files']) && count($field['files']) > 0 ) {
                        ...
                        match ($typeclass) {
                            ...
                            'File', 'Image' => self::checkFileImageFieldPermissions($em, $pm_service, $user, $data_record, $data_field, $orig_dataset, $field),
                            default => throw new ODRBadRequestException('Structure for File/Image fields used on '.$typeclass.' Field '.$data_field->getFieldUuid(), $exception_source),
                        };
                    }
...
//                                && $record['template_uuid'] == $o_record['template_uuid']
                    ...
                    if ( !$record_found ) {
                        ...
                        if ( $datatree->getIsLink() ) {
                            ...
                            self::checkUpdatePermissions($record, [], $user, false, $changed);
                        }
                    }
                }
            }
...
//                                    throw $e;
                            ...
                            $dataset['records'][$i] = self::datasetDiff($record, [], $user, false, $changed);
                        }
            ...
            if ( !empty($link_status_changed_records) ) {
                ...
                foreach ($link_status_changed_records as $dr) {
                    ...
                    if ( !isset($events[$dt_id]) )
                        $events[$dt_id] = ['descendant_datatype' => $dr->getDataType(), 'records' => []];
...
//                        throw new ODRException('unable to locate image "'.$file['file_uuid'].'", dr "'.$datarecord->getId().'", df "'.$datafield->getId().'"');
                ...
                if ( $typeclass === 'File' ) {
                    $file['href'] = $router->generate('odr_file_download', ['file_id' => $file_obj->getId()], UrlGeneratorInterface::ABSOLUTE_URL);
        ...
        foreach ($field['files'] as $i => $img) {
            ...
            if ( isset($relevant_images[$img_uuid]) ) {
                ...
                $new_image_data = [
                    ...
                    'href' => $router->generate('odr_image_download', ['image_id' => $relevant_img->getId()], UrlGeneratorInterface::ABSOLUTE_URL),
    ...
    private function updateStorageField($em, $dispatcher, $user, $datarecord, $datafield, $orig_dataset, $field)
    {
        ...
        if ($fields_updated) {
            ...
            if ( $typeclass === 'DatetimeValue' ) {
                ...
                $new_storage_entity->setValue( mb_scrub((string) $field['value']) );
            }
...
//                    throw $e;
     ...
     * @return Response
     */
    public function getRecordsByDatasetUUIDAction($version, $dataset_uuid, $record_uuid = null, $limit = 100, $offset = 0, ?Request $request = null): Response
    {
     ...
     *     body: { "user_email": "..." }   (user_email optional; defaults to the authenticated user)
     *   DELETE /api/{version}/datarecord
     *     body: { "user_email": "...", "record_uuids": ["uuid1", "uuid2", ...] }
     *
     * Both forms reject more than 100 records per request. Permission is checked per-record via
     * PermissionsManagementService::canDeleteDatarecord; if the caller lacks permission on *any*
     * record in the batch, the whole request is rejected before any deletion happens (fail-closed).
     * Acting as another user (via user_email) requires ROLE_SUPER_ADMIN. Patterned after
     * deleteDatasetByUUIDAction. Ported from develop 878400fd + 2be0b76f.
     *
     * @param string      $version
    ...
    public function deleteDatarecordByUUIDAction($version, $datarecord_uuid, Request $request)
    {
        try {
            ...
            // Content is optional when a datarecord_uuid is supplied in the URL
            $content = $request->getContent();
            $data = json_decode($content, true);
            ...
            if ( $user_email === '' ) {
                ...
                // Acting as another user requires super-admin permissions
                throw new ODRForbiddenException();
            }
            ...
            // Build the working list of UUIDs from URL path + body, merged + deduped
            $uuids = array();
            if ( $datarecord_uuid !== null && $datarecord_uuid !== '' )
                $uuids[] = $datarecord_uuid;

            if ( is_array($data) && isset($data['record_uuids']) && is_array($data['record_uuids']) ) {
            ...
            // Resolve every record + check every permission BEFORE deleting anything. If any check
            //  fails, the whole batch is rejected -- a partial delete on a forbidden record would be
            //  hard to undo (fail-closed).
            $repo = $em->getRepository('ODR\AdminBundle\Entity\DataRecord');
            $records = array();
    ...
    public function deleteDatasetByUUIDAction($version, $dataset_uuid, Request $request)
    {
        ...
        try {
            ...
            if (!empty($content)) {
                ...
                /** @var ODRUser $user */
                $user = $user_manager->findUserBy(['email' => $data['user_email']]);
                if (is_null($user))
    ...
    private function parse_raw_http_request($a_data = [])
    {
        ...
        preg_match('/boundary=(.*)$/', (string) $_SERVER['CONTENT_TYPE'], $matches);
...
//                    throw $e;
            ...
            // Switching to get datarecord which uses user's permissions to build array
            // This is required because the user can turn databases non-public.
            $url = $this->generateUrl('odr_api_get_dataset_single_no_format', [
                'version' => $version,
...
//                    $logger->debug('unable to move file to "'.$destination_folder.'/'.$original_filename.'"???', array('APIController::addfileAction()'));
    ...
    public function setRecordFieldSelectionAction($version, $record_uuid, $field_uuid, $option_uuid, $state, Request $request)
    {
        try {
            ...
            /** @var DataRecord $datarecord */
            $datarecord = $em->getRepository('ODR\AdminBundle\Entity\DataRecord')->findOneBy(array('unique_id' => $record_uuid));
            if ($datarecord === null || $datarecord->getDeletedAt() !== null)
                throw new ODRNotFoundException('Datarecord');

            /** @var DataFields $datafield */
            $datafield = $em->getRepository('ODR\AdminBundle\Entity\DataFields')->findOneBy(array('fieldUuid' => $field_uuid));
            ...
            if ($typeclass === 'Tag') {
                /** @var Tags $tag */
                $tag = $em->getRepository('ODR\AdminBundle\Entity\Tags')->findOneBy(
                ...
                if ($selected && ($typename === 'Single Radio' || $typename === 'Single Select')) {
                    $existing_selections = $em->getRepository('ODR\AdminBundle\Entity\RadioSelection')->findBy(
            ...
            if (
                ...
                foreach ($field->value as $option_id => $option) {
                    foreach ($option as $key => $selected_option) {
                        if (preg_match("/\s\&gt;\s/", (string) $selected_option->name)) {
                            // We need to split and process
                            $option_data = preg_split("/\s\&gt;\s/", (string) $selected_option->name);
                            for ($i = 0; $i < count($option_data); $i++) {
                                if ($i == 0) {
                                    if (!isset($options_data[$option_data[0]])) {
                                        $options_data[$option_data[0]] = [
                                            'count' => 0,
                                        ];
                                    }
                                ...
                                if ($i == 1) {
                                    if (!isset($options_data[$option_data[0]][$option_data[1]])) {
                                        $options_data[$option_data[0]][$option_data[1]] = [
                                            'count' => 0,
                                        ];
                                    }
                                ...
                                if ($i == 2) {
                                    if (!isset($options_data[$option_data[0]][$option_data[1]][$option_data[2]])) {
                                        $options_data[$option_data[0]][$option_data[1]][$option_data[2]] = [
                                            'count' => 0,
                                        ];
                                    }
                                ...
                                if ($i == 3) {
                                    if (!isset($options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]])) {
                                        $options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]] = [
                                            'count' => 0,
                                        ];
                                    }
                                ...
                                if ($i == 4) {
                                    if (!isset($options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]][$option_data[4]])) {
                                        $options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]][$option_data[4]] = [
                                            'count' => 0,
                                        ];
                                    }
                                ...
                                if ($i == 5) {
                                    if (!isset($options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]][$option_data[4]][$option_data[5]])) {
                                        $options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]][$option_data[4]][$option_data[5]] = [
                                            'count' => 0,
                                        ];
                                    }
                                ...
                                if ($i == 6) {
                                    if (!isset($options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]][$option_data[4]][$option_data[5]][$option_data[6]])) {
                                        $options_data[$option_data[0]][$option_data[1]][$option_data[2]][$option_data[3]][$option_data[4]][$option_data[5]][$option_data[6]] = [
                                            'count' => 0,
                                        ];
                                    }
                            ...
                            if (!isset($options_data[$selected_option->name])) {
                                $options_data[$selected_option->name] = [
                                    'count' => 0,
                                ];
                            }
            ...
            if($tracked_job) {
                ...
                for($i=0; $i<$num_jobs; $i++) {
                    ...
                    */
                    ...
                    $worker_job->setRandomKey('random_key_' . random_int(0,1000000));
