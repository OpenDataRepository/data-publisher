 ...
 * storage_entity_cleanup tube and passes the parameters to WorkerController,
...
class StorageEntityCleanupCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        ...
        $this
            ->setName('odr_record:storage_entity_cleanup')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_record:storage_entity_cleanup] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
...
//$logger->debug('MassEditCommand.php: curl results...'.print_r($result, true));
                ...
                if ( $e->getMessage() == 'retry' ) {
                    ...
                    $logger->error('StorageEntityCleanupCommand.php: '.$e->getMessage());
