...
class CloneDatatypePreloaderMonitorCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        ...
        $this
            ->setName('odr_datatype:clone_datatype_preloader_monitor')
            ->setDescription('Restarts the odr_datatype:clone_datatype_preloader process after it exits.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while (true) {
            ...
            if(count($pids) == 1) {
                // start job
                exec("php app/console odr_datatype:clone_datatype_preloader >> app/logs/clone_datatype_preloader.log 2>&1 &");
                $current_time = new \DateTime();
                $output->writeln( 'Restarting clone_datatype_preloader: ' . $current_time->format('Y-m-d H:i:s').' (UTC-5)' );
            }
