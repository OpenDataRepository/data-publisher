...
class CSVExportExpressFinalizeCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_csv_export:finalize_express] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
            $job = null;
            try {
                // Wait for a job?
                ...
                if ( $e->getMessage() == 'retry' ) {
                    ...
                    $logger->error('CSVExportExpressFinalizeCommand.php: '.$e->getMessage());
