...
class CloneAndLinkDatatypeMonitorCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        ...
        $this
            ->setName('odr_datatype:clone_and_link_monitor')
            ->setDescription('Restarts the odr_datatype:clone_and_link process after it exits.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while (true) {
            ...
            $pids = array();
            exec("ps auxww |grep 'odr_datatype:clone_and_link_datatype'", $pids);

            $output->writeln( 'PIDS OUTPUT: ' . var_export($pids));

            if(count($pids) == 2) {
                // start job
                exec("php app/console odr_datatype:clone_and_link_datatype >> app/logs/clone_and_link_datatype.log 2>&1 &");
                $current_time = new \DateTime();
                $output->writeln( 'Restarting clone_and_link_datatype: ' . $current_time->format('Y-m-d H:i:s').' (UTC-5)' );
            }
