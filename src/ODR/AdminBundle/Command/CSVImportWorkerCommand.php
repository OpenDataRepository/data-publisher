...
class CSVImportWorkerCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_csv_import:worker] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
            $job = null;
            try {
                // Wait for a job?
                ...
                // 
                $str = 'CSV Import request for DataType '.$data->datatype_id.' from '.$data->redis_prefix.'...';
...
//$logger->debug('CSVImportWorkerCommand.php: curl results...'.print_r($result, true));
                ...
                if ( $e->getMessage() == 'retry' ) {
                    ...
                    $logger->error('CSVImportWorkerCommand.php: '.$e->getMessage());
