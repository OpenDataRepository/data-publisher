...
class XMLImportWorkerCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_xml_import:worker] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
            $job = null;
            try {
                // Wait for a job
...
//$logger->debug('XMLImportWorkerCommand.php: curl results...'.print_r($result, true));
                ...
                if ( $e->getMessage() == 'retry' ) {
                    ...
                    $logger->error('XMLImportWorkerCommand.php: '.$e->getMessage());
