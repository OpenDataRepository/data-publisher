...
class ImportWorkerCommand extends ContainerAwareCommand
{
...
//                $job = $pheanstalk->watch($memcached_prefix.'_import_datarecord')->ignore('default')->reserve();
                ...
                $str = '';
                if ($data->datatype_id !== '')
                    $str = 'ImportWorkerCommand.php: Attempting to import "'.$data->xml_filename.'" for DataType '.$data->datatype_id.' from '.$data->memcached_prefix.'...';
                else if ($data->datafield_id !== '')
                    $str = 'ImportWorkerCommand.php: Attempting to import "'.$data->xml_filename.'" for DataField '.$data->datafield_id.' from '.$data->memcached_prefix.'...';
                else
                ...
                $output->writeln($str);
                $logger->info('ImportWorkerCommand.php: '.$str);
...
//$logger->debug('ImportWorkerCommand.php: curl results...'.print_r($result, true));
                ...
                $logger->err('ImportWorkerCommand.php: '.$e->getMessage());
