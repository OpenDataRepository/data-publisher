...
class ClearStorageEntityCleanupCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        ...
        $this
            ->setName('odr_record:clear_storage_entity_cleanup')
            ->setDescription('Removes jobs for deleting useless blank storage entities')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_record:clear_storage_entity_cleanup] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
            // Wait for a job?
            if ($input->getOption('old'))
                $job = $pheanstalk->watch($redis_prefix.'_storage_entity_cleanup')->ignore('default')->reserve();
            else
...
if ($input->getOption('old'))
    $output->writeln( date('H:i:s').'  '.$str.' from '.$redis_prefix.'_storage_entity_cleanup');
else
    $output->writeln( date('H:i:s').'  '.$str.' ('.$job_source.') from storage_entity_cleanup');
