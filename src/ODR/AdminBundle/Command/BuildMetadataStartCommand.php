 ...
 * BuildMetadataStart Command
 * (C) 2015 by Nathan Stone (nate.stone@opendatarepository.org)
 * (C) 2015 by Alex Pires (ajpires@email.arizona.edu)
 * Released under the GPLv2
 *
 * This Symfony console command takes beanstalk jobs from the
 * build_metadata tube and passes the parameters to WorkerController
 * to create a given entities' metadata entry.
 */
...
class BuildMetadataStartCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_metadata:start] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
            ...
            try {
                ...
                $output->writeln('Metadata build request...');
...
//$logger->debug('BuildMetadataStartCommand.php: curl results...'.print_r($result, true));
                ...
                if ( $e->getMessage() == 'retry' ) {
                    ...
                    $logger->error('BuildMetadataStartCommand.php: '.$e->getMessage());
