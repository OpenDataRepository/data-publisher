...
class TagRebuildCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('[odr_record:tag_rebuild] started '.(new \DateTime())->format('Y-m-d H:i:s').' — waiting for jobs (Ctrl+C to stop)');
        while (true) {
...
//$logger->debug('TagRebuildCommand.php: curl results...'.print_r($result, true));
                ...
                if ( $e->getMessage() == 'retry' ) {
                    ...
                    $logger->error('TagRebuildCommand.php: '.$e->getMessage());
