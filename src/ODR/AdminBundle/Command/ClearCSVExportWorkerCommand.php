    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while (true) {
            ...
            if ($input->getOption('old'))
                $job = $pheanstalk->watch($memcached_prefix.'_csv_export_worker')->ignore('default')->reserve();
            else
...
if ($input->getOption('old'))
    $output->writeln( date('H:i:s').'  deleted job for datarecord '.$datarecord_id.' from '.$memcached_prefix.'_csv_export_worker');
else
