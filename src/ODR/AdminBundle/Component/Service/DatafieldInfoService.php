...
class DatafieldInfoService
{
    ...
    public function getDatafieldProperties($datatype_array, $datafield_id = null)
    {
        ...
        // ----------------------------------------
        // Load properties for all datafields by default, or a single datafield if defined TODO
...
//                        'public_status_message' => $public_status_message,
    ...
    /**
     * Helper function to determine whether a datafield can be safely deleted.
     *
     * @param array $datatype_array
    ...
    public function canDeleteDatafield($datatype_array, $datatype_id, $datafield_id)
    {
        ...
        if ( !is_null($dtm['metadataNameField']) && $dtm['metadataNameField']['id'] === $datafield_id ) {
            return array(
                'can_delete' => false,
                'delete_message' => "This datafield can't be deleted because it's being used as the Metadata Datatype's name field"
            );
        }
        if ( !is_null($dtm['metadataDescField']) && $dtm['metadataDescField']['id'] === $datafield_id ) {
            return array(
                'can_delete' => false,
                'delete_message' => "This datafield can't be deleted because it's being used as the Metadata Datatype's description field"
            );
        }
    ...
    /**
     * Helper function to determine whether a datafield can have its public status changed.
     *
     * @param array $datatype_array
     * @param int $datatype_id
     * @param int $datafield_id
    ...
    public function canChangePublicStatus($datatype_array, $datatype_id, $datafield_id)
    {
        ...
        // Can't change public status of a metadata name/desc field
        if ( !is_null($dtm['metadataNameField']) && $dtm['metadataNameField']['id'] === $datafield_id ) {
            return array(
                'can_change_public_status' => false,
//                'public_status_message' => "This datafield must remain public because it's being used as the Metadata Datatype's name field"
            );
        }
        if ( !is_null($dtm['metadataDescField']) && $dtm['metadataDescField']['id'] === $datafield_id ) {
            return array(
                'can_change_public_status' => false,
//                'public_status_message' => "This datafield must remain public because it's being used as the Metadata Datatype's description field"
            );
        }


        // ----------------------------------------
        // Otherwise, no problems changing public status of this field
        return array(
...
//            'public_status_message' => '',
    ...
    public function getAllowedFieldtypes($datafield, $datatype_array = null)
    {
        ...
        if ( $dt['dataTypeMeta']['renderPlugin']['pluginClassName'] !== 'odr_plugins.base.default' ) {
            ...
            // ...then if this datafield is being used by the render plugin...
            foreach ($rpi['renderPluginMap'] as $rpm) {
