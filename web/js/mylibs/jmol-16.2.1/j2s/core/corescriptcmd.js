...
Clazz_declarePackage("JS");
Clazz_load(["JS.ScriptExt"], "JS.CmdExt", ["java.util.Hashtable", "JU.AU", "$.BS", "$.Base64", "$.Lst", "$.M3", "$.M4", "$.Measure", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.STER", "$.VDW", "J.i18n.GT", "JM.AtomCollection", "$.LabelToken", "$.Measurement", "$.Text", "JS.SV", "$.ScriptCompiler", "$.ScriptError", "$.ScriptEval", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.T", "JU.BSUtil", "$.BoxInfo", "$.C", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.Parser", "$.Point3fi", "$.SimpleUnitCell", "JV.FileManager", "$.StateManager", "$.Viewer"], function(){
...
default:
var ret =  new Array(1);
if (this.getUnitCellParameter(ret)) {
oabc = ret[0];
} else {
newUC = ret[0];
...
if (sym == null) {
this.vwr.assignSpaceGroup(null, "P1", newUC);
...
isReset = (isModelkit ? this.vwr.getModelkit(false).transformAtomsToUnitCell(sym, oabc, ucname) : true);
if (isReset) {
...
type = this.e.optParameterAsString(i);
if (type.equals("ITA")) {
if (this.e.optParameterAsString(i + 1).equals("/")) {
type += "/" + this.e.optParameterAsString(i + 2);
this.e.iToken = i = i + 2;
} else {
type += "/";
...
this.vwr.getModelkit(false).cmdAssignAtom(bs, pt, type, this.e.fullCommand, isClick);
break;
case 1677721602:
...
break;
case 134217764:
var s = this.vwr.assignSpaceGroup(bs, type, paramsOrUC);
...
function(ret){
ret[0] = null;
if (this.tokAt(this.e.iToken + 1) == 4) {
var tr = this.paramAsStr(++this.e.iToken);
...
try {
if (this.tokAt(this.e.iToken + 2) != 1073742332) ret[0] = this.e.floatParameterSet(this.e.iToken, 6, 6);
...
this.e.ignoreError = false;
ret[0] = this.e.getPointArray(this.e.iToken, 4, false);
}return true;
}, "~A");
...
;//5.0.1-v2 Fri Mar 15 16:28:59 CDT 2024
})(Clazz
,Clazz.getClassName
,Clazz.newLongArray
,Clazz.doubleToByte
