...
Clazz.load(["JU.Vibration", "JU.BS", "$.P3", "J.i18n.GT"], "J.modelkit.ModelKit", ["java.util.Arrays", "$.Hashtable", "JU.AU", "$.Lst", "$.M4", "$.Measure", "$.PT", "$.Quat", "$.SB", "$.V3", "JS.SV", "JU.BSUtil", "$.Edge", "$.Elements", "$.Logger", "JV.Viewer"], function(){
...
this.addAtoms(element,  Clazz.newArray(-1, [(ptNew != null ? ptNew : null)]), JU.BSUtil.newAndSetBit(atomIndex), "", "click", true);
}, "~N,~S,JU.P3");
Clazz.defineMethod(c$, "cmdAssignAddAtoms", 
function(type, pts, bsAtoms, packing, cmd, isClick){
return this.addAtoms(type, pts, bsAtoms, packing, cmd, isClick);
}, "~S,~A,JU.BS,~S,~S,~B");
Clazz.defineMethod(c$, "cmdAssignAtom", 
function(bs, pt, type, cmd, isClick){
if (pt != null && bs != null && bs.cardinality() > 1) bs = JU.BSUtil.newAndSetBit(bs.nextSetBit(0));
this.assignAtoms(pt, -1, bs, type, (pt != null), cmd, isClick, 0, 0, null, null, null);
}, "JU.BS,JU.P3,~S,~S,~B");
...
function(bs, name, paramsOrUC){
return this.assignSpaceGroup(bs, name, paramsOrUC);
}, "JU.BS,~S,~O");
...
if (isPoint) {
for (var i = 0; i < pts.length; i++) this.assignAtoms(pts[i], -1, null, type, true, cmd, false, 1, -1, null, null, "");

return pts.length;
}this.assignAtoms(pts[0], atomIndex, null, type, true, cmd, false, 1, -1, null, null, "");
return 1;
}var bsM = this.vwr.getThisModelAtoms();
...
this.assignAtoms(JU.P3.newP(pts[i]), atomIndex, bsEquiv, stype, true, null, false, atomicNo, site, sym, list, packing);
}
} else {
var sites =  new JU.BS();
...
this.assignAtoms(JU.P3.newP(a), -1, null, stype, false, null, false, atomicNo, site, sym, list, packing);
...
function(pt, atomIndex, bs, type, newPoint, cmd, isClick, atomicNo, site, sym, points, packing){
...
sym.getEquivPointList(points, nIgnored, packing + (newPoint && atomIndex < 0 ? "newpt" : ""));
...
this.assignAtom(atomIndex, type, this.autoBond, sym == null, true, bsEquiv);
...
if (points == null) {
this.assignAtom(atomIndexNew, type, false, atomIndex >= 0 && sym == null, true, null);
if (atomIndex >= 0) {
var doAutobond = (sym == null && !"H".equals(type));
this.assignAtom(atomIndex, ".", false, doAutobond, isClick, null);
...
}, "JU.P3,~N,JU.BS,~S,~B,~S,~B,~N,~N,J.api.SymmetryInterface,JU.Lst,~S");
...
} else if (name.indexOf('.') > 0 && !Float.isNaN(JU.PT.parseFloat(name))) {
isITA = true;
...
var sym = this.vwr.getOperativeSymmetry();
var params = (paramsOrUC == null || !JU.AU.isAD(paramsOrUC) ? null : paramsOrUC);
if (sym == null) {
sym = this.vwr.getSymTemp().setUnitCellFromParams(params == null ?  Clazz.newFloatArray(-1, [10, 10, 10, 90, 90, 90]) : params, false, NaN);
paramsOrUC = null;
} else if (paramsOrUC != null) {
if (JU.AU.isAD(paramsOrUC)) params = paramsOrUC;
 else params = this.vwr.getSymTemp().getUnitCell(paramsOrUC, false, "assign").getUnitCellParams();
...
var basis;
var sg = null;
var origin = sym.getUnitCellMultiplied().getUnitCellVectors()[0];
var sgInfo = (noAtoms && !isDefined ? null : this.vwr.findSpaceGroup(isDefined ? null : bsAtoms, isDefined ? (isITA ? "ITA/" + name : name) : null, params == null ? sym.getUnitCellMultiplied().getUnitCellParams() : params, origin, false, true, false));
if (sgInfo == null) {
if (isITA) {
...
if (this.isElementKeyOn(i)) this.bsElementKeyModels.set(i);
}
});
